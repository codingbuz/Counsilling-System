...
class StudReg extends JFrame implements ActionListener, KeyListener, ItemListener
{
...
//  con=DriverManager.getConnection("jdbc:odbc:Driver={Microsoft Access Driver (*.mdb)};");                 
	...
	StudReg()
	{
		...
		JLabel lb1=new JLabel("Registration Form",JLabel.CENTER);
		...
		try
		{
		  ...
		  ResultSet rs1=st1.executeQuery("SELECT REG_NO FROM STUD_REG");
		  int i=0;
		  while(rs1.next())
		  {
		  i=rs1.getInt(1);
		  } 
		  reg.setText(Integer.toString(++i));
		  rs1.close();
		}catch(SQLException e)
		{
...
//IN ("+Integer.parseInt(ch1.getText())+","+Integer.parseInt(ch2.getText())+","+Integer.parseInt(ch3.getText())+")");
...
System.out.println("((((");
			  ...
			  {
			  PreparedStatement ps=con.prepareStatement("INSERT INTO STUD_REG VALUES(?,?,?,?,?,?,?,?,?,?,?)");
			  ps.setInt(1,Integer.parseInt(reg.getText()));
			  ps.setString(2,rno.getText());
			  ps.setInt(3,Integer.parseInt(rnk.getText()));
			  ps.setString(4,nam.getText());
			  ps.setString(5,add.getText());
			  ps.setString(6,num.getText());
			  ps.setString(7,stcd);
			  ps.setString(8,cnm.getText());
			  ps.setInt(9,Integer.parseInt(ch1.getText()));
			  ps.setInt(10,Integer.parseInt(ch2.getText()));
			  ps.setInt(11,Integer.parseInt(ch3.getText()));
			  int i=ps.executeUpdate();
			  ps.close();
			  if(i==0)
			  JOptionPane.showMessageDialog(null,"Insertion Error","Message",0);
			  else
